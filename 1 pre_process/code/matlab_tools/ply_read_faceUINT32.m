function [ Elements, varargout ] = ply_read_faceUINT32 ( Path, Str )
...
%
      if ( ~any(Type) )
        ...
        for k = 1 : NumProperties
          ListData{k} = uint32(zeros(ElementCount(i),3));
        end
%
% list type
%
        for j = 1 : ElementCount(i)
          for k = 1 : NumProperties
            ...
            if ( ~Type(k) )
              ...
              tmp = Buf(BufOff);
              ListData{k}(j,:) = uint32(Buf(BufOff+(1:tmp))');
              BufOff = BufOff + tmp + 1;
            end
...
%
              SkipNum = Buf(Miss(1));
              j = j + 1;
              ListData{1}(j,:) = uint32(fread(fid,[1,SkipNum],Type2{1}));
...
%
  ...
  clear Data
  clear ListData;
%   Elements.face.vertex_indices = uint32(cell2mat(Elements.face.vertex_indices));
...
%
      if ( ~iscell(PropertyName) )
        ...
        Elements = FaceIndices.'+1;
        
        
% Le code ci-dessus => erreur        
%         N = length(FaceIndices);
%         Elements = uint32(zeros(3,N*2));
%         Extra = 0;
% 
%         for k = 1 : N
% 
%           Elements(1:3,k) = uint32(FaceIndices{k}(1:3)');
% %
% %  The original code had an error in the following loop.
% %
%           for j = 4 : length(FaceIndices{k})
%             Extra = Extra + 1;
%             Elements(1,N + Extra) = uint32(FaceIndices{k}(1));
%             Elements(2,N + Extra) = uint32(FaceIndices{k}(j-1));
%             Elements(3,N + Extra) = uint32(FaceIndices{k}(j));
%           end
% 
%         end
% %
